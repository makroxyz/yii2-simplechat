...
class Conversation extends ActiveRecord
{
    ...
    public static function get($userId, $limit, $history = true, $key = null)
    {
        ...
        if (null !== $key) {
            $query->andHaving([$history ? '<' : '>', new Expression('MAX([[id]])'), $key]);
        }
    ...
    {
        if (static::getDb()->driverName === 'mysql') {
            $isDeleteBySenderExpression = new Expression('CASE WHEN ([[sender_id]] = :userId) THEN TRUE ELSE is_deleted_by_sender END');
            $isDeleteByReceiverExpression = new Expression('CASE WHEN ([[receiver_id]] = :userId) THEN TRUE ELSE is_deleted_by_receiver END');
        } elseif (static::getDb()->driverName === 'pgsql') {
            $isDeleteBySenderExpression = new Expression('IF ([[sender_id]] = :userId, TRUE, is_deleted_by_sender)');
            $isDeleteByReceiverExpression = new Expression('IF ([[receiver_id]] = :userId, TRUE, is_deleted_by_receiver)');
        }
        
        $count = static::updateAll([
            'is_deleted_by_sender' => $isDeleteBySenderExpression,
            'is_deleted_by_receiver' => $isDeleteByReceiverExpression
        ], ['or',
